# Fuente: http://www.inference.org.uk/mackay/python/compress/
#! /usr/bin/python
...
def iterate (c) :
    ...
    """
    if ( len(c) > 1 ) :
        c.sort() ## sort the nodes by count, using the __lt__ function defined in the node class
...
def encode(sourcelist,code):
    ...
    for s in sourcelist:
        ...
        if ( not co  ):
            import sys
            print( "Warning: symbol" +str(s)+" has no encoding!")
            pass
        else:
...
def decode(string,root):
    ...
    for c in clist:
        ...
        if currentnode.leaf != 0:
            answer.append( currentnode.name )
            currentnode = root
        pass
...
def huffman_uncompress(binstring, root):
    """
    Input:
        * binstring: string de 0/1
        * root: árbol de Huffman
    Output: lista de simbolos
    """
    return decode(binstring,root)
